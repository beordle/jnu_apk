...
#     if i.has_key('value'):
    ...
    """" 被注释掉的是下载xls"""
    """
  s=re.findall(r'&ReportID=(.*?)&ControlID', html, re.S)

  iframe=soup.find("iframe")
  url=urljoin("http://jwc.jnu.edu.cn",iframe['src'])
  result=urlparse(url)
  params=parse_qs(result.query,True)
  url= params['ReportUrl'][0]
  url=urljoin("http://jwc.jnu.edu.cn",url)
  result=urlparse(url)
  params=parse_qs(result.query,True)
  a= s[2]
  b= params['ControlID'][0]
#      Referer[http://jwc.jnu.edu.cn/web/areaMain.aspx]



  url="http://jwc.jnu.edu.cn/Web/Reserved.ReportViewerWebControl.axd?Mode=true&ReportID=%s&ControlID=%s&Culture=2052&UICulture=2052&ReportStack=1&OpType=Export&FileName=Rpt_Student_Kcb&ContentDisposition=AlwaysAttachment&Format=Excel" %(a,b)
  xls=urllib2.urlopen(url).read()
  open(stuid+".xls",'wb').write(xls)
  """
